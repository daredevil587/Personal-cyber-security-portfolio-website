        ...
        >
          {info.map((item, i) => (
            <motion.div
              ...
              className="flex items-start gap-4 p-4 rounded-lg border border-[#1f2937] bg-[#111827] hover:border-blue-500/30 transition-colors"
            >
              <span className="font-mono text-xs text-blue-400 tracking-widest w-28 shrink-0 mt-0.5">{item.label}</span>
              <span className="text-[#e5e7eb] text-sm">{item.value}</span>
            </motion.div>
          ))}
        </motion.div>
      </div>
